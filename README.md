...
يحتوي المشروع على 4 توزيعات مخصصة لكل منصة تشغيل:

1. **`B:\VM-Portable` (النسخة الأصلية الشاملة):** النسخة المحمولة الكاملة المحتوية على استوديو الترجمة والدبلجة وكافة أدوات الإنتاج.
2. **`B:\VM-PC` (نسخة الكمبيوتر المجهزة):** نسخة حواسيب سريعة ومصممة بأداء عالٍ (بدون تبويب الدبلجة).
3. **`B:\VM-Mobile` (نسخة الجوال اللمسية):** نسخة متجاوبة للمس مع واجهة مرنة للهواتف الذكية والأجهزة اللوحية.
4. **`B:\VM-Web` (نسخة الويب — VIPD.SHOP):** تطبيق ويب مستقل يستند لـ Express Server ويدعم التحميل المباشر من المتصفح.
